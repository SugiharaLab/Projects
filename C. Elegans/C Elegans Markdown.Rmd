...
```{r}
  load("totals_foraging_final.RData")
...
```{r}
load("/Users/eriksaberski/Desktop/foraging_Es_final.RData")
par(mfrow=c(1,4))
breaks = seq(0,42,3)
for(i in c(1:4)){
...
```{r, warning=F}
load("totals_escaping_final.RData")
  par(mfrow = c(4,4), mai = c(.3,.3,.3,.3))
  for(i in c(1:4)){
    for(j in c(1:4)){
      if(i != j){
...
```{r, eval = F}
  ...
  count = 1
  load("/Users/eriksaberski/Desktop/escape_totals_tn.RData")
  for(n1 in c(1:length(nums))){
    n = nums[n1]
    found = F
...
The processed data (and processing scripts) to run the following is available at https://github.com/SugiharaLab. 

First, we average the CCM profiles of individuals with the same genotype.
```{r}
genes = list.files("worms_csv")
genes_found = {}
for(g in genes){
  files = list.files(paste("worms_csv/",g,sep = ""))
  for(f in files){
    if(grepl( "CCM.RDat", f,fixed = TRUE)){
      load(paste("worms_csv/",g,"/",g,"_CCM.RData", sep = ""))
      if(length(totals)!=0){
        genes_found = c(genes_found, g)
      }
    }
  }
}
genes_found = genes_found[-26]
all_genes = array(NA, dim = c(length(genes_found), 4,4,11))
count = 0
...
```{r}
phenos = read.csv("sd01.csv") #load in the data file from Brown et al. (2013)
...
```{r}
test = read.csv("new_mutations2.csv", header = F)
